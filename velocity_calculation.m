...
HeightScaleBar=[4.4,4.4,4.4,4.4];% unit is cm. This is the scale reference for videos in each subfolder.
%%
...
for im =1:numberOfFolders-1 %im is the subfolder number
    ...
    scale=HeightScaleBar(im)/BoundingBoxs(4);
    ...
    trial=cell(1,rec-1);
    r0=cell(1,rec-1);
    actv{1,im}=zeros(frame_withdraw_number,(rec-1)+1+(rec-1)/3);% if three trials were performed for both DM Forms I and II.
    ...
    for iv=1:frame_withdraw_number
        ...
        I0=read(video,1+(iv-1)*speed_up-frame_number(ive-1));
    ...
    %%
    for idx=2:rec
        actv_velocity{1,im}(:,idx)=scale*actv{1,im}(:,idx)/(20*(total_area_per_trial(idx-1)/20)^0.5);
    end
    %%
    for ir=1:(rec-1)/3
        actv{1,im}(:,rec-1+1+ir)=(actv{1,im}(:,ir*3-1)+actv{1,im}(:,ir*3)+actv{1,im}(:,ir*3+1))/3;%average of all trials
        actv_velocity{1,im}(:,rec-1+1+ir)=(actv_velocity{1,im}(:,ir*3-1)+actv_velocity{1,im}(:,ir*3)+actv_velocity{1,im}(:,ir*3+1))/3;
    end
    %%
    actv_m=actv{1,im};
